...
class BinotelAPI:
    ...
    def get_free_slots(self, date_str):
        ...
        # КРИТИЧНО: branchId як STRING для підпису
        ...
        try:
            ...
            # ВИВОДИМО ПОВНИЙ ТЕКСТ ПОМИЛКИ ДЛЯ ДІАГНОСТИКИ
...
def resolve_target_date(text):
    today = datetime.now()
    if "завтр" in text.lower(): return (today + timedelta(days=1)).strftime("%Y-%m-%d")
    return today.strftime("%Y-%m-%d")

def build_system_prompt(target_date, crm_data):
    if crm_data == "ALL_BUSY":
        instr = f"На {target_date} вільних місць немає. Попроси клієнта обрати іншу дату або зачекати адміністратора."
    elif crm_data == "NO_DATA":
        instr = "Зараз дані розкладу недоступні. Скажи, що адміністратор напише особисто найближчим часом. Не вигадуй час."
    else:
        instr = f"Ось реальний розклад на {target_date}: {crm_data}. Пропонуй тільки ці вікна."
...
def process_message(sender_id, text):
    ...
    with user_lock:
        ...
        try:
            ...
            reply = response.choices[0].message.content or "Адмін скоро напише вам!"
            session["messages"].append({"role": "assistant", "content": reply})
            send_instagram_msg(sender_id, reply)
            log("Відповідь надіслана.")
        except Exception as exc: log(f"OpenAI error: {exc}")
...
def webhook():
    ...
    for entry in data.get("entry", []):
        for event in entry.get("messaging", []):
            ...
            if sender_id and text: threading.Thread(target=process_message, args=(sender_id, text), daemon=True).start()
